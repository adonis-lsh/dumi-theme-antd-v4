...
const DocAnchor: React.FC<DocAnchorProps> = ({ showDebug, debugDemos = [] }) => {
  ...
  const renderAnchorLink = (item: AnchorItem): React.ReactNode => {
    ...
    return (
      <Anchor.Link key={item.id} href={`#${item.id}`} title={item.title}>
        {children}
      </Anchor.Link>
    );
  };
  ...
  return (
    <section css={styles.tocWrapper}>
      <Anchor css={styles.toc} affix={false} targetOffset={token.anchorTop} showInkInFixed>
